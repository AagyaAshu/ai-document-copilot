...
def ingest_accessions(
    ...
    *,
    ...
    # pool_pre_ping: check a connection is alive before reusing it, and
    # transparently open a fresh one if not. Needed because Supabase's
    # Session Pooler can close idle connections while we're mid-batch on
    # slow embedding calls (rate-limit pacing keeps this session open for
    # a while between DB writes).
